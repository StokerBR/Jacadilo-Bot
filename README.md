# Jacadilo-Bot 🐊
Jacadilo Bot é um bot de Discord feito exclusivamente para o meu servidor. Ele possui comandos úteis e divertidos, e está atualmente na versão _1.9.3_ 🤖

> O bot foi feito em JavaScript ![js](https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/187_Js_logo_logos-512.png =12x) com ![node.js](https://upload.wikimedia.org/wikipedia/commons/thumb/7/7e/Node.js_logo_2015.svg/1280px-Node.js_logo_2015.svg.png =70x) e utilizando a biblioteca ![discord.js](https://www.pngfind.com/pngs/b/102-1026997_jeffy-discordjs-discord-js-logo-hd-png-download.png =75x).
> Ele está hospedado em São Paulo em uma instância EC2 da Amazon Web Services ![aws](https://upload.wikimedia.org/wikipedia/commons/thumb/9/93/Amazon_Web_Services_Logo.svg/1280px-Amazon_Web_Services_Logo.svg.png =20x).
## Comandos 💬
> O prefixo do bot é: `jacadilo `

[`aniversário`](#aniversário), [`análise`](#análise), [`apague`](#apague), [`buzzfeed`](#buzzfeed), [`calado`](#calado), [`cante`](#cante), [`chame`](#chame), [`comandos`](#comandos), [`copypaste`](#copypaste), [`dance`](#dance), [`entre`](#entre), [`furry`](#furry), [`imagem`](#imagem), [`imite`](#imite), [`info`](#info), [`jacadilo`](#jacadilo), [`leaderboard`](#leaderboard), [`oi`](#oi), [`quantidade`](#quantidade), [`reinicie`](#reinicie), [`rule34`](#rule34), [`saia`](#saia), [`spam`](#spam), [`vídeo`](#vídeo)
### aniversário
- **Descrição:**
_Mostra todos os aniversários dos integrantes do server que foram adicionados ao comando. No dia do aniversário da pessoa (às 00h) manda um "Feliz aniversário" no chat Geral_

- **Uso:**
_``jacadilo aniversário``, ``jacadilo aniversário add @<pessoa> <dia> <mês>``, ``jacadilo aniversário apague @<pessoa>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_Administrador (para adicionar ou apagar)_
### análise
- **Descrição:**
_Envia a imagem do jacadilo análise_

- **Uso:**
_``jacadilo análise``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### apague
- **Descrição:**
_Apaga a quantidade determinada de mensagens (até 20)_

- **Uso:**
_``jacadilo apague <quantidade>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_Apagar mensagens_
### buzzfeed
- **Descrição:**
_Envia um teste do buzzfeed aleatório_

- **Uso:**
_``jacadilo buzzfeed``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### calado
- **Descrição:**
_Para de cantar_

- **Uso:**
_``jacadilo calado``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### cante
- **Descrição:**
_Toca "interior crocodile alligator"_

- **Uso:**
_``jacadilo cante``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### chame
- **Descrição:**
_Envia uma mensagem direta para a pessoa que foi marcada falando que você está chamando ela. Também envia uma mensagem escrita por você, caso inserida, juntamente com o chamado_

- **Uso:**
_``jacadilo chame @<pessoa>``, ``jacadilo chame @<pessoa> <mensagem>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### comandos
- **Descrição:**
_Envia a lista de comandos ou descreve um comando específico_

- **Uso:**
_``jacadilo comandos``, ``jacadilo comandos <comando>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### copypaste
- **Descrição:**
_Envia um copypaste aleatório_

- **Uso:**
_``jacadilo copypaste``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### dance
- **Descrição:**
_Envia um gif dancando_

- **Uso:**
_``jacadilo dance``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### entre
- **Descrição:**
_Entra no canal de voz que o usuário está_

- **Uso:**
_``jacadilo entre``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### furry
- **Descrição:**
_Envia uma imagem de um furry aleatório do site _This Fursona Does Not Exist__

- **Uso:**
_``jacadilo furry``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### imagem
- **Descrição:**
_Envia uma imagem aleatória sobre o termo(s) de pesquisa inserido, ou a foto de perfil da pessoa marcada_

- **Uso:**
_``jacadilo imagem <termo de pesquisa>``, ``jacadilo imagem @<pessoa>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### imite
- **Descrição:**
_Imita uma pessoa ou uma frase_

- **Uso:**
_``jacadilo imite <argumento>``, ``jacadilo imite isso <frase>``_

- **Argumentos:**
_``mudo``, ``jacadilo``, ``sombra``, ``mãe do edésio``, ``mãe do moraes``_

- **Permissão Necessária:**
_-_
### info
- **Descrição:**
_Forneçe informação sobre o que for pedido_

- **Uso:**
_``jacadilo info <argumento>``_

- **Argumentos:**
_``versão``, ``criador``, ``github``, ``jacadilo``_

- **Permissão Necessária:**
_-_
### jacadilo
- **Descrição:**
_Envia o emoji jacadilo_

- **Uso:**
_``jacadilo jacadilo``, ``jacadilo jacadilo jacadilo``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### leaderboard
- **Descrição:**
_Envia o Leaderboard das pessoas que mandaram mais jacadilos no canal jacadilo_

- **Uso:**
_``jacadilo leaderboard``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### oi
- **Descrição:**
_Responde "Eae"_

- **Uso:**
_``jacadilo oi``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### quantidade
- **Descrição:**
_Diz quantas vezes você ou a pessoa que foi marcada já mandou "jacadilo" no canal jacadilo. Também informa quantos jacadilos lendários você já conseguiu_

- **Uso:**
_``jacadilo quantidade``, ``jacadilo quanditade @<pessoa>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### reinicie
- **Descrição:**
_Reinicia o bot_

- **Uso:**
_``jacadilo reinicie``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_Administrador_
### rule34
- **Descrição:**
_Envia uma imagem aleatória do site _Rule34_ sobre o termo(s) de pesquisa inserido_

- **Uso:**
_``jacadilo rule34 <termo de pesquisa>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### saia
- **Descrição:**
_Sai do canal de voz_

- **Uso:**
_``jacadilo saia``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
### spam
- **Descrição:**
_Envia "jacadilo" periodicamente_

- **Uso:**
_``jacadilo spam <argumento>``_

- **Argumentos:**
_``ligado``, ``desligado``, ``ligado lento``, ``status``_

- **Permissão Necessária:**
_Administrador_
### vídeo
- **Descrição:**
_Envia um vídeo aleatório do youtube sobre o termo(s) de pesquisa inserido_

- **Uso:**
_``jacadilo vídeo <termo de pesquisa>``_

- **Argumentos:**
_-_

- **Permissão Necessária:**
_-_
